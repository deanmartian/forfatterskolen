...
const methods = {
    ...
    addNote : function(el){
        ...
        form_group.find(".chapter-prompt").hide();
        let get_chapter = $(el).prop('id').replace('add-note-chp-',"");
        self.chapter_id = parseInt(get_chapter);
    ...
    saveFeedback : function(el, published){
        ...
        let data = { id : elem.feedback_id, chapter_id : self.chapter_id, published : published, message : converter.makeHtml(editors[elem.key].getData()) };
        ...
        if (!$(el).parent('.form-group').prev('.note-list-container').length) {
            prev_el = $(el).parent('.form-group').closest('.chapter-feedback-item').find('.note-list-container');
        }
        $.post(url, data)
    ...
    cancelNote : function(el){
        ...
        let chapter_feedback_item = $(el).closest(".chapter-feedback-item");
        if (chapter_feedback_item.find(".chapter-prompt").length) {
            chapter_feedback_item.find(".chapter-prompt").show();
        }
    ...
    listFeedback: function(el, feedback, parent_el, prev_el) {
        ...
        let new_item = `
                        <div class="form-group mb-0 clearfix">
                            ...
                            <a class="pull-right edit-note-btn" id="chp-${ this.chapter_id }">
        ...
        if (is_update) {
            ...
            if (parent_el.closest(".chapter-feedback-item").find("ul").length === 0) {
                parent_el.closest(".chapter-feedback-item").find(".chapter-prompt").remove();
                let new_list = "<ul class='list-group note-list-container'>";
                        new_list += "<li class='list-group-item clearfix' id='note-li-"+feedback.id+"'>";
                            new_list += new_item;
                        new_list += "</li>";
                    new_list += "</ul>";
                parent_el.closest(".chapter-feedback-item").find('.full-messages').append(new_list);
            } else {
    ...
    editFeedback: function(el, feedback) {
        ...
        let get_chapter = $(el).prop('id').replace("chp-", "");
        self.chapter_id = parseInt(get_chapter);
