...
Route::prefix('v1')->middleware(['cors', 'apiRequestId'])->group(function () {
    ...
    Route::middleware('apiJwt')->group(function () {
        ...
        Route::get('/learner/invoice/{id}/receipt/view', [InvoiceController::class, 'receiptView'])
            ->name('api.learner.invoice.receipt.view');
...
| Helpwise Webhook
|--------------------------------------------------------------------------
| POST = mottak av webhook-events fra Helpwise
*/
Route::post('/helpwise/webhook', [\App\Http\Controllers\Api\HelpwiseWebhookController::class, 'handle']);
