...
class HelpwiseApiService
{
    ...
    {
        try {
            $response = $this->client()->get("{$this->baseUrl}/v1/conversations", $params);

            if ($response->successful()) {
                return $response->json();
            }

            Log::error('Helpwise listConversations failed', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            ...
            Log::error('Helpwise listConversations exception', ['message' => $e->getMessage()]);
            return null;
        }
    }
