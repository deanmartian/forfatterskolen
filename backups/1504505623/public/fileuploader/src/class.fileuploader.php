...
class FileUploader
{
    ...
    private function validate($file = null)
    {
        if ($file == null) {
            ...
            if (($this->options['limit'] && $this->options['limit'] < $this->field['count'] + count($this->options['files'])) || ($ini[3] != 0 && ($this->field['count']) > $ini[3])) {
            ...
            if ($this->options['fileMaxSize'] && $this->options['fileMaxSize'] < $file['size'] / 1000000) {
