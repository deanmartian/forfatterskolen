				  ...
				  <div id="passwordreset" class="tab-pane fade @if(Request::input('t') == 'passwordreset') in active @endif">
					<form method="post" action="{{route('frontend.passwordreset.store')}}">
						...
						<div class="form-group">
							<div class="input-group">
							  	<span class="input-group-addon" id="sizing-addon1"><i class="fa fa-at"></i></span>
								<input type="email" name="reset_email" placeholder="Epost" class="form-control no-border-left" required value="{{old('reset_email')}}">
							</div>
						</div>
						<button type="submit" class="btn btn-primary pull-right">Tilbakestill passord lenke</button>
						<div class="clearfix"></div>
					</form>
				  </div>
				  <span class="margin-top" style="display: inline-block;">
				  Logg deg inn på det gamle systemet (gjelder for all som har registrert seg før 1.7.16). <br />
