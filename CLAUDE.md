...
- **Bemanningsplan for kurs** — Ny `course_staff`-tabell og `CourseStaff`-modell med roller: editor, mentor, guest_editor, course_leader, webinar_host. Ny "Bemanningsplan"-side i kurs-sidemenyen (`?section=staffing`) med tre seksjoner: (1) Kursroller — CRUD for kursholder, mentor, gjesteredaktør med datoer og notater. (2) Webinar-plan — alle webinarer for kurset med vert-dropdown (auto-submit), forbi webinarer dimmet, denne uken uthevet, bulk-tildel alle uten vert. (3) Redaktør per elev — dropdown per elev, bulk-tildel alle uten redaktør, kapasitetsoversikt (antall elever per redaktør). Controller-metoder i `CourseController`: storeStaff, deleteStaff, assignEditor, bulkAssignEditor, assignWebinarHost, bulkAssignWebinarHost.
- **Årskurs Høst 2026 (kurs 133)** — Opprettet via artisan-kommando `course:create-arskurs-host-2026` som kopierer kurs 119 (Vår 2026) med nye datoer tilpasset høst/vinter: oppstart 31. august, 6 oppgaver med frister som unngår høstferie/juleferie/vinterferie/påske, 39 webinarer tirsdager kl 12:00. Kursplan-leksjon oppdatert med riktige datoer.
- **Nyhetsbrev for Motor-webinar** — Artisan-kommando `newsletter:create-motor-webinar` oppretter invitasjonsmail planlagt søndag 12. april kl 19:00, segment `no_active_course` (alle uten aktive elever). Emne: "Hvorfor stopper romanen din opp?" med CTA til `/gratis-webinar/95`.
- **CRM/Nyhetsbrev synlige knapper** — `btn-outline-*` er usynlig i Bootstrap 3 (admin bruker BS3). Byttet alle til `btn-xs btn-primary`/`btn-default` i sekvens-show og newsletter-index. Også lagt til "Legg til nytt steg"-modal på sekvens-siden.
